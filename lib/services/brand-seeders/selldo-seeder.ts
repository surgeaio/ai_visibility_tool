...
/**
 * Standalone citation seeder — inserts 19 rows into the citations table if
 * none exist for this brand. Safe to call multiple times (idempotent).
 * Called by both seedSellDoDemoData (step 6) and ensureCitationsExist.
...
export async function seedSellDoDemoData(brandId: string): Promise<SellDoSeedResult> {
  ...
  await seedSellDoCitations(brandId);
